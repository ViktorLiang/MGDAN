...
from dataset.transforms import cv2_transforms
from utils.transforms import get_affine_transform
from dataset.datasets_utils import dataset_pathes

class LIPDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[448, 448], scale_factor=0.25,
                 rotation_factor=45, ignore_label=255, transform=None, num_classes=20,
                 dataset_name="", label_edge=True):
        """
        :rtype:
        """
        ...
        if self.dataset != 'val':
            ...
            if self.dataset == 'train' or self.dataset == 'trainval':
                ...
                if random.random() <= self.flip_prob:
                    ...
                    if random.random() <= 0.6: #connect with its non-flip version
                        im = np.concatenate([im_src, im], axis=1)
                        parsing_anno_src = cv2.imread(parsing_anno_path, cv2.IMREAD_GRAYSCALE)
                        parsing_anno = np.concatenate([parsing_anno_src, parsing_anno], axis=1)
                ...
                if random.random() <= self.gray_prob:
                    im = cv2_transforms.to_gray(im)
                #channel shuffle
                if random.random() <= self.channel_shuffle_prob:
                    im = cv2_transforms.channel_shuffle(im)
                #median blur
