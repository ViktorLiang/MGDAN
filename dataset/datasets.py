...
from dataset.target_generation import generate_edge, generate_canny_edge, generate_onehot_edge
...
class LIPDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[473, 473], scale_factor=0.25,
        ...
        :rtype:
        """
        assert dataset_name in ['PASCAL_PERSON', 'LIP', 'CIHP']
    ...
    def __getitem__(self, index):
        ...
        # img_id, bboxes = self.person_box_list[index]['img_id'], self.person_box_list[index]['bboxes']
        im_path = os.path.join(self.root, self.dataset + '/JPEGImages', im_name + '.jpg')
        parsing_anno_path = os.path.join(self.root, self.dataset + '/parsing_anno', im_name + '.png')
        ...
        if self.dataset != 'test':
            ...
            if self.dataset == 'train' or self.dataset == 'trainval':
                ...
                if random.random() <= self.flip_prob:
                    ...
                    if self.dataset_name in ['LIP', 'CIHP']:
                        right_idx = [15, 17, 19]
                        left_idx = [14, 16, 18]
                        for i in range(0, 3):
